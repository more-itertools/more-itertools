...
* Changes to existing functions:
    * :func:`collapse`, :func:`chunked_even`, :func:`ichunked`, :func:`padded`, and :func:`windowed` were optimized (thanks to james-wasson).
...
* Changes to existing functions:
    ...
    * :func:`sliced` had its type hint updated (thanks to ad-chaos).
...
* Changes to existing itertools:
    * :func:`numeric_range` now supports ranges specified by ``datetime.datetime`` and ``datetime.timedelta`` objects (thanks to MSeifert04 for tests).
...
The major version update is due to the change in the default behavior of :func:`collapse`. It now treats ``bytes`` objects the same as ``str`` objects. This aligns its behavior with :func:`always_iterable`.
...
* Major changes:
    ...
    * The :func:`accumulate` function is no longer part of this library. You may import a better version from the standard ``itertools`` module.

* Changes to existing itertools:
    * The order of the parameters in :func:`grouper` have changed to match the latest recipe in the itertools documentation. Use of the old order will be supported in this release, but emit a  ``DeprecationWarning``. The legacy behavior will be dropped in a future release (thanks to jaraco).
...
The major version update is due to the change in the default return value of :func:`first_true`. It's now ``None``.
...
* New itertools:
    ...
    * :func:`make_decorator` - see the blog post `Yo, I heard you like decorators <https://sites.google.com/site/bbayles/index/decorator_factory>`_ for a tour (thanks to pylang).
...
* Improvements to existing itertools:
    ...
    * The performance tradeoffs between :func:`roundrobin` and :func:`interleave_longest` are now documented (thanks michael-celani, pylang, and MSeifert04).
...
* Improvements to existing itertools:
    ...
    * ``peekable``-wrapped iterables are now indexable and slicable. Iterating through ``peekable``-wrapped iterables is also faster.
